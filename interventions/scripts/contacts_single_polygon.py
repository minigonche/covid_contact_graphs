...
report_name  =  sys.argv[2] # location id
...
plt.savefig(os.path.join(export_folder_location, f"hourly_stats_{control_polygon_name}.png"))
...
print(indent + "\tSaving control dataset.")
df_contacts_control.to_csv(os.path.join(export_folder_location, f"contacts_{control_polygon_name}.csv"), index=False)

df_contacts = df_contacts_control.drop(columns=["contacts_agg"])

# Plot contact change
print(indent + "\tPlotting...")
df_contacts_plot = pd.melt(df_contacts, id_vars=['date'], value_vars=['percentage_change_ctrl'])
...
print(indent + "\tExporting...")
plt.savefig(os.path.join(export_folder_location, f"percent_change_contacts_{control_polygon_name}.png"))
